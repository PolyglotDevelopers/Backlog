    ...
    public class EmployeeController : BaseController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                switch (status)
                {
                    case RegistrationResultEnum.InvalidEmpCode:
                        ModelState.AddModelError("", "Unable to register the employee");
                        break;
                }
        ...
        public async Task<IActionResult> Edit(EmployeeModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var entity = await _employeeService.GetByIdAsync(model.Id);
        ...
        private async Task InitModelAsync(EmployeeRegistrationModel model)
        {
            ...
            foreach (var item in currencies)
            {
                model.AvailableCurrencies.Add(new SelectListItem
                {
                    Text = item.Name,
                    Value = item.Id.ToString(),
                    Selected = item.Id == model.Employee.CurrencyId
                });
            }
