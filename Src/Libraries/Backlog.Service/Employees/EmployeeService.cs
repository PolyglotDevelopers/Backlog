...
namespace Backlog.Service.Employees
{
    public class EmployeeService : IEmployeeService
    {
        ...
        {
            var prefix = await _settingsService.GetStringValueByNameAsync(ServiceConstant.EmployeePrefix);
            var sequence = await _settingsService.GetStringValueByNameAsync(ServiceConstant.EmployeeSequence);
            if (string.IsNullOrEmpty(prefix))
                return null;
            int.TryParse(sequence, out var lastSequence);
            ...
            bool emailWelcomeKit)
        {
            ...
            if (newAddressEntity.Id > 0)
            {
                ...
                employee.CurrencyId = loggedEmployee.CurrencyId;
